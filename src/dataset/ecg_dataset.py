    ...
    def _sample_mask(self, n_channels: int, rng: np.random.Generator) -> torch.Tensor:
        ...
        Supported strategies:
          random / random_independent:
              each lead gets independent random masked patch indices.
          random_synchronized:
              both leads get the same random masked patch indices.
          block:
              both leads get the same contiguous temporal block.
        """
